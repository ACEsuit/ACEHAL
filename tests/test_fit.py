...
def test_fit_properties(fit_data, fit_model):
    ...
    assert np.sqrt(np.mean(test_diffs['F'] ** 2)) > 0.001
    assert np.sqrt(np.mean(test_diffs['V'] ** 2)) > 0.05

def test_fit_smooth_properties(fit_data, fit_model_smooth):
    calc = fit_model_smooth
    fit_configs, test_configs, E0s, data_keys, _ = fit_data
    props = ['E', 'F', 'V']
    ...
    print("fit RMS Fs", np.sqrt(np.mean(fit_diffs['F'] ** 2)))
    print("fit RMS Vs", np.sqrt(np.mean(fit_diffs['V'] ** 2)))
    assert np.sqrt(np.mean(fit_diffs['E'] ** 2)) < 0.0003
    assert np.sqrt(np.mean(fit_diffs['F'] ** 2)) < 0.0007
    assert np.sqrt(np.mean(fit_diffs['V'] ** 2)) < 0.01
    ...
    assert np.sqrt(np.mean(test_diffs['E'] ** 2)) > 0.001
    assert np.sqrt(np.mean(test_diffs['F'] ** 2)) > 0.001
    assert np.sqrt(np.mean(test_diffs['V'] ** 2)) > 0.03
